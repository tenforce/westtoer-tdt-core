    ...
    public function read() {
        ...
        if (!isset($this->export_package)) {
            ...
            if ($model->hasPackage($this->export_package)) {
                ...
                $exception_config["log_dir"] = Config::get("general", "logging", "path");
                $exception_config["url"] = Config::get("general", "hostname") . Config::get("general", "subdir") . "error";
                throw new TDTException(452, array("package, " . $this->export_package . ", not found"), $exception_config);
            }
        } else {
            if ($model->hasResource($this->export_package, $this->export_resource)) {
                ...
                $exception_config["log_dir"] = Config::get("general", "logging", "path");
                $exception_config["url"] = Config::get("general", "hostname") . Config::get("general", "subdir") . "error";
                throw new TDTException(452, array("package-resource, " . $this->export_package . "/" . $this->export_resource . ", not found."), $exception_config);
            }
        }
