...
Class XmlUtils {
    ...
    public static function xmlToObject($xml, $obj = null) {
        ...
        foreach ($xml->children() as $xmlChild) {
            ...
            if (is_array($obj->$nodeName)) {
                ...
                // Invoke recursion
                self::xmlToObject($xmlChild, $objArray[$idx]);
            }
            // If child is object
            if (is_object($obj->$nodeName)) {
                ...
                // Invoke recursion
                self::xmlToObject($xmlChild, $obj->$nodeName);
            }
            if(!isset($arrayIdx[$nodeName])){
                $arrayIdx[$nodeName] = 0;
            }
    ...
    public static function xmlFileToObject($xmlFileName) {
        //if (!file_exists($xmlFileName)) die ("self::xmlFileToObject Error: $xmlFileName nonexistent!");
        $xml = simplexml_load_file($xmlFileName);

        if(!$xml)
            return $xml;

        return self::xmlToObject($xml);
    }
}
